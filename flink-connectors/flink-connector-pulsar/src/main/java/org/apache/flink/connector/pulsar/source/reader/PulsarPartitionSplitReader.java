...
public class PulsarPartitionSplitReader<T>
    ...
    public RecordsWithSplitIds<ParsedMessage<T>> fetch() {
        wakeup = false;
        PulsarPartitionSplitRecords<ParsedMessage<T>> recordsBySplits =
                new PulsarPartitionSplitRecords<>();
    ...
    public void handleSplitsChanges(SplitsChange<PulsarPartitionSplit> splitsChange) {
        ...
        try {
            AsyncUtils.parallelAsync(
                    splitsChange.splits(),
                    this::createPartitionReaderAsync,
                    (partition, reader) -> readerQueue.add(reader),
    ...
    private void reportDataLoss(PartitionRange partition, String error) {
        String fullError =
                String.format(
                        ...
                        partition, error, PulsarSourceOptions.VERIFY_INITIAL_OFFSETS.key());
