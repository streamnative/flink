...
public class PulsarSourceEnumerator
    ...
    public void addSplitsBack(List<PulsarPartitionSplit> splits, int subtaskId) {
        splitSchedulingStrategy.addSplitsBack(
                pendingPartitionSplitAssignment, splits, subtaskId, context.currentParallelism());
    ...
    public void close() {
        ...
        close(closeTimeoutMs)
                .ifPresent(
                        t -> LOG.warn("Encountered error when closing PulsarSourceEnumerator", t));
    }
                        ...
                        .map(
                                partition ->
                                        new PulsarPartitionSplit(
                                                partition, startOffsetInitializer, stopCondition))
    ...
    private void addPartitionSplitChangeToPendingAssignments(
        ...
        for (PulsarPartitionSplit split : newPartitionSplits) {
            // TODO: Implement a more sophisticated algorithm to reduce partition movement when
            // parallelism changes.
