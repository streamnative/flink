...
/**
 * An implementation of {@link StartOffsetInitializer} which initializes the offsets of the
 * partition according to the user specified offsets.
 *
...
public class SpecifiedStartOffsetInitializer implements StartOffsetInitializer {
    ...
    public Optional<String> verifyOffset(
        ...
        if (!lastMessageId.isPresent()) {
            return Optional.of(
                    String.format(
                            "Cannot initialize to offset %s because topic is empty", initialId));
        }

        if (initialId.compareTo(lastMessageId.get()) > 0) {
            return Optional.of(
                    String.format(
                            "The initial offset %s is beyond the last commit message id",
                            initialId));
        }
        ...
        if (inclusive) {
            ...
            if (!firstMessage.get().getMessageId().equals(nextInitialId)) {
                return Optional.of(
                        String.format(
                                "Unexpected offset %s, but expected %s",
                                firstMessage.get().getMessageId(), nextInitialId));
            }
        }
