...
public abstract class BasePulsarSubscriber implements PulsarSubscriber {
    ...
    protected List<TopicPartition> createPartitions(
            TopicMetadata metadata, TopicRangeGenerator rangeGenerator) {
        if (!metadata.isPartitioned()) {
            // For non-partitioned topic.
            return rangeGenerator.range(metadata.getName(), 0, false)
                    .stream()
                    .map(range -> new TopicPartition(metadata.getName(), 0, false, range))
                    .collect(toList());
        } else {
            ...
            return IntStream.range(0, metadata.getPartitionSize())
                    ...
                    .flatMap(
                            partitionId ->
                                    rangeGenerator.range(metadata.getName(), partitionId, true)
                                            .stream()
                                            .map(
                                                    range ->
                                                            new TopicPartition(
