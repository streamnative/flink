...
public class PulsarSourceEnumeratorStateSerializer
    ...
    public byte[] serialize(PulsarSourceEnumeratorState enumState) throws IOException {
        return SerdeUtils.serializeSplitAssignments(
                enumState.getCurrentAssignment(), new PulsarPartitionSplitSerializer());
    }

    @Override
    public PulsarSourceEnumeratorState deserialize(int version, byte[] serialized)
            throws IOException {
        if (version == 0) {
            Map<Integer, List<PulsarPartitionSplit>> currentPartitionAssignment =
                    SerdeUtils.deserializeSplitAssignments(
                            serialized, new PulsarPartitionSplitSerializer(), ArrayList::new);
